...
type SignatureScheme interface {
	...
	// attributes: as described above;
	// rhIndex: revocation handle index relative to attributes;
	// eidIndex: index of the enrollment ID relative to attributes;
	// skIndex: index of the secret key relative to attributes;
	// revocationPublicKey: revocation public key;
	// epoch: revocation epoch.
	// verType: type of verification.
	// meta: additional verification metadata.
	...
	AuditNymEid(ipk IssuerPublicKey, eidIndex, skIndex int, signature []byte, enrollmentID string, RNymEid *math.Zr, verType AuditVerificationType) error

	// AuditNymRh permits the auditing of the nym rh generated by a signer
	AuditNymRh(ipk IssuerPublicKey, rhIndex, skIndex int, signature []byte, revocationHandle string, RNymRh *math.Zr, verType AuditVerificationType) error
}
...
type NymSignatureScheme interface {
	...
	Verify(pk IssuerPublicKey, Nym *math.G1, signature, digest []byte, skIndex int) error
}
